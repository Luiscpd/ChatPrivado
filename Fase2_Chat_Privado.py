...
import math
import os 
import serial #Libreria para establecer comunicación en baudios
import time #Para trabajar en función del tiempo

#-----------------------------------------------------------------------------------------------------------------------------
opcion = 0
if __name__ == '__main__':
    ser = serial.Serial('/dev/ttyS0', 57600, timeout=1) #Se establece puerto de comunicacion y baudios
...
while opcion != 3:
    ...
    if opcion == 1: #SI LA OPCION ES IGUAL A UNO NOS PIDE EL MENSAJE QUE SE VA A ENVIAR PARA CIFRARLO
            os.system('cls')
            texto = input("INTRODUCIR MENSAJE > ").upper() 
            n = 5 #Llave para el cifrado cesar, la dejanamos fija porque consideramos que una aplicacion de mensajeria no nos debe de pedir colocarla, pero en dado caso se pudiera colocar como un variable de ingreso.  
            ...
            print("--------------------------------------------------------------")
			#AQUI INICIA TOMANDO EL MENSAJE CIFRADO EN CESAR Y LO INICIA A CIFRAR EN HILL
        ...
        #ESTA ES NUSTRA CLAVE PARA EL CIFRADO HILL, IGUALMENTE COLOCAMOS UNA POR DEFAULT PORQUE CONSIDERAMOS QUE NO DEBE DE PEDIRSE AL USUARIO INGRESAR UNA 
            Clave[0][0] = 11;
            Clave[0][1] = 8;
            Clave[1][0] = 3;
            Clave[1][1] = 7;
            ...
            print(salida)
            print("--------------------------------------------------------------")
			#NUESTRO MENSAJE CIFRADO EN HILL Y CESAR NOS LO CIFRA EN BINARIO
...
#COMINICACION UART 
    ...
    if opcion == 2: #SI SE ELIGE LA OPCION DOS EL PROGRAMA SE COLOCA EN MODO DE ESCUCHA ESPERANDO RECIBIR ALGUN MENSAJE
        while True:
            if ser.in_waiting > 0:
    ...
    #            else:
                    #Descifrar
                    # Solicitar Datos
					#CUANDO EL PROGRAMA DETECTA UN MENSAJE RECIBIDO INICIA LA DECODIFICACION
                os.system('cls')
                Texto = line;
...
#UTILIZA LA MISMA CLAVE CON LA QUE SE CODIFICO EL MENSAJE PARA DECODIFICARLO
                Clave[0][0] = 11;
                Clave[0][1] = 8;
                Clave[1][0] = 3;
                Clave[1][1] = 7;
...
#TOMA EL MENSAJE YA DECIFRADO DE HILL Y LO INICIA A DESIFRAR EN CESAR
